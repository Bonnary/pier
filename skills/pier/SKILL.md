...
| `[deploy.<env>]` | `host`, `user`, `path`, `branch`, optional `port` (SSH port, default 22), optional `services` override, `domain` (HTTPS via Caddy; empty = plain HTTP), `redirect_domains` (served and redirected to `domain`), `ports` overrides, optional `queue_workers` (absent = inherit `[stack]`), `before_deploy` / `after_deploy` hook lists. |
| `[deploy.<env>].builder` | Where the image is built: `host_server` (default), `local_machine`, or `build_server` (needs `build_host` / `build_user` / `build_path`, optional `build_port` default 22). |
