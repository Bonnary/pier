...
func (s *Stack) GenerateProdFiles(cfg config.Config, env string) (stack.Files, error) {
	...
	trustedProxies := renderTrustedProxiesConfig()
...
func prodEnvForServices(services []string) map[string]string {
	...
	// TRUSTED_PROXIES feeds config/trustedproxy.php (see
	// renderTrustedProxiesConfig): the Caddy webserver terminates TLS and
	// proxies plain HTTP, so Laravel must trust it to generate https://
	// URLs. Interpolated so users can pin a specific proxy from
	// .env.production instead of the '*' default.
...
func renderProdEnv(cfg config.Config, env string, services []string) []byte {
	...
	// Caddy terminates TLS and proxies plain HTTP to the app, so Laravel
	// must trust it to generate https:// URLs (see
	// config/trustedproxy.php). '*' trusts all proxies; override with your
	// proxy IP/subnet if the app is ever published directly.
	fmt.Fprintln(&b, "TRUSTED_PROXIES=*")
	fmt.Fprintln(&b)
	set := map[string]bool{}
	for _, s := range services {
		set[s] = true
...
// renderTrustedProxiesConfig returns the config/trustedproxy.php file.
// The webserver (Caddy) terminates TLS and reverse-proxies plain HTTP to
// the app (php artisan serve), and Laravel 13 no longer applies APP_URL
// to URL generation, so without trusting the proxy the app renders
// http:// asset URLs that browsers block as mixed content (white screen).
// Laravel's default TrustProxies middleware reads the trustedproxy.proxies
// key; '*' is the Laravel Forge/Cloud convention for an app that is not
// directly reachable (pier never publishes the app port). Users can pin a
// proxy IP/subnet via TRUSTED_PROXIES in .env.production.
func renderTrustedProxiesConfig() []byte {
	return []byte(`<?php

// Generated by pier. The webserver (Caddy) terminates TLS and forwards
// plain HTTP to the app, so Laravel must trust its proxy headers to
// generate https:// URLs. '*' trusts all proxies — pier never publishes
// the app port, so only the webserver can reach it. For stricter
// security set TRUSTED_PROXIES in .env.production to your proxy's IP or
// subnet.
return [
